...
class RequestScheduler:
    ...
    def __init__(self):
        ...
        self.max_concurrent_requests = 10  # 可配置
        self.task_request_cap = 5  # 任务型请求的最大并发数，可根据系统负载动态调整
        self.latency_threshold = 500  # 毫秒，延迟阈值，超过则减少任务型请求
    ...
    async def handle_realtime_request(self, db_request, model_provider_service, db):
        ...
        try:
            ...
            # 动态调整任务型请求的上限：如果有实时请求，减少任务型请求
            if self.active_realtime_requests > 0:
                self.task_request_cap = max(2, self.max_concurrent_requests - self.active_realtime_requests)
            
            logger.info(f"Processing realtime request {db_request.request_id}")
    ...
    async def update_system_stats(self, db):
        ...
        stats = models.SystemStats(
            ...
            avg_latency=0.0,  # 可以从历史请求中计算
            throughput=0.0  # 可以从历史请求中计算
        )
        ...
        if stats.avg_latency > self.latency_threshold:
            ...
            logger.info(f"High latency detected, reducing task cap to {self.task_request_cap}")
